...
def predict(model_id,guest_name,proj_dir,model_type=None):
    ...
    for month_i in month_lst:
        ...
        if model_type:
            with open(PREDICT_DIR+'predict_lr_fdn.json', 'rb') as fs:
        ...
        predict['role_parameters']['guest']['args']['data']['train_data_0'][0]['namespace'] = 'hzph'
        json_data = json.dumps(predict,indent=4)
        if model_type:
            with open('/data/projects/fate/python/auto_model/predict_model/predict_lr_fdn.json', 'w') as fw:
                fw.write(json_data)
            fw.close()
            log_pre = os.popen(
                """source /data/projects/fate/bin/init_env.sh && cd /data/projects/fate/python/auto_model/predict_model/ && sh s_predict_lr_fdn.sh""")
        else:
